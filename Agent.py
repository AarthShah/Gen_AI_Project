...
agent = create_agent(model=llm,tools=[get_Sunbeam_data_tool,get_file_contain_tool_by_path,weather_details_tool,calculater_tool,get_user_uploaded_file_data_tool],system_prompt="You are a helpful AI assistant mainly focused on Sunbeam InfoTech. Use the provided tool to answer questions related to Sunbeam InfoTech, its courses, internships, placements, pre-cat information, and contact details. If the question is not related to Sunbeam InfoTech, try to ans from Your Database if u cant then politely inform the user.but u can also ans general questions if asked.")
